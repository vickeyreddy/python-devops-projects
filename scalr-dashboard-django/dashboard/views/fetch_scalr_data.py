...
##### Configure Django to Connect to SQLiteDB #####
	...
	for fk,fv in farms.iteritems():
		...
		if servers == None:
			...
			for sk, sv in servers.iteritems():
				upt=(float(sv[4]/60/60))
				srv = Servers(sid=sk, ename=enm, fname=fv[1], frname=fv[2], sip=sv[2], s_status=sv[3], uptime=str(upt), s_type=sv[5])
				srv.save()
				print ("Server sucessfully %s %s!!!" % (sk, sv[2]))


##### Fetching Servers Details from specific environments, farms and farmroles #####
def get_extended_serverinfo(eid,sid):
	data = c.fetch_server_info(sid=sk)
	return data
